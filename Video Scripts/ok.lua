...
-- https://ok.ru/video/4138886498843
	...
	local userAgent = 'Mozilla/5.0 (Windows NT 10.0; Win64; x64; rv:155.0) Gecko/20100101 Firefox/155.0'
	...
	if not retAdr and answer:match('/videoembed/') then
		url = answer:match('"embedUrl"%shref="([^"]+)')
		local rc, answer = m_simpleTV.Http.Request(session, {url = url})
			if rc ~= 200 then return end
		retAdr = answer:match('videos&quot;:%[%{&quot;%a+&quot;:&quot;%a+&quot;,&quot;url&quot;:&quot;(.-)&quot;')
	end
		if not retAdr then
	...
	local title
	if not(answer:match('"vid%-card_live __active">Live<') and m_simpleTV.Control.ChannelID ~= 268435455)
	then
		local addTitle = 'OK'
		local answer = answer:gsub('\\\\\\&quot;', '"')
		title = answer:match('<title>(.-)</title>'):gsub('&quot;', '')
		if not title then
			title = addTitle
		else
			if m_simpleTV.Control.MainMode == 0 then
				...
				local logo = answer:match('"og:image"%scontent="([^"]+)')
				if logo then
