-- видеоскрипт для плейлиста "Акадо ТВ" https://akado.tv (13/1/26)
-- Copyright © 2017-2026 Nexterr, NEKTO666 | https://github.com/Nexterr-origin/simpleTV-Scripts
-- ## необходим ##
-- скрапер TVS: akadotv_pls.lua
-- ## открывает подобные ссылки ##
-- https://akado.tv/66c324ee9a7b3e72cc96e80b
		if m_simpleTV.Control.ChangeAddress ~= 'No' then return end
		if not m_simpleTV.Control.CurrentAddress:match('^https?://akado%.tv')
		then return end
	...
	local lastQuality = tonumber(m_simpleTV.Config.GetValue('akado_qlty') or 20000)
	local index = #t
	if #t > 1 then
		t[#t + 1] = {}
		t[#t].Id = 20000
		...
		if m_simpleTV.Control.MainMode == 0 then
			...
			t.ExtParams = {LuaOnOkFunName = 'akadoSaveQuality'}
	...
	function akadoSaveQuality(obj, id)
		m_simpleTV.Config.SetValue('akado_qlty', id)
	end
