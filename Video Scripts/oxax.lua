-- видеоскрипт для плейлиста "ОХ-АХ" http://oxax.tv (3/4/26)
...
-- http://oxax.tv/oh-ah.html
	...
	local v1 = answer:match('kodk="([^"]+)') or ''
	local v2 = answer:match('kos="([^"]+)') or ''
	retAdr = retAdr:gsub('{v1}', v1):gsub('{v2}', v2)
	if retAdr:match('%sor%s') then
		retAdr = retAdr:match('^https://s.oxax.tv/[^%s]+')
	end
