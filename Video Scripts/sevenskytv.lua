-- видеоскрипт для плейлиста "SevenSky TV" https://tv.seven-sky.net (24/2/26)
-- Copyright © 2017-2026 Nexterr, NEKTO666 | https://github.com/Nexterr-origin/simpleTV-Scripts
-- ## необходим ##
-- скрапер TVS: sevenskytv_pls.lua
-- ## открывает подобные ссылки ##
-- https://tv.seven-sky.net/569f94a0685e040a2a08fc08
		if m_simpleTV.Control.ChangeAddress ~= 'No' then return end
		if not m_simpleTV.Control.CurrentAddress:match('^https?://tv%.seven%-sky%.net')
		then return end
	...
	local lastQuality = tonumber(m_simpleTV.Config.GetValue('svnsktv_qlty') or 20000)
	local index = #t
	if #t > 1 then
		t[#t + 1] = {}
		t[#t].Id = 20000
		...
		if m_simpleTV.Control.MainMode == 0 then
			...
			t.ExtParams = {LuaOnOkFunName = 'svnsktvSaveQuality'}
	...
	function svnsktvSaveQuality(obj, id)
		m_simpleTV.Config.SetValue('svnsktv_qlty', id)
	end
