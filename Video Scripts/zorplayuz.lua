-- видеоскрипт для плейлиста "ZOR UZ" https://zorplay.uz (15/12/25)
-- Copyright © 2017-2025 Nexterr, NEKTO666 | https://github.com/Nexterr-origin/simpleTV-Scripts
-- ## необходим ##
-- скрапер TVS: zorplay_pls.lua
-- ## открывает подобные ссылки ##
-- https://zorplay.uz/channels/uzreporttv/680
		if m_simpleTV.Control.ChangeAddress ~= 'No' then return end
		if not m_simpleTV.Control.CurrentAddress:match('^https?://zorplay%.uz') then return end
	...
	local function GetToken()
		...
		local rc, answer = m_simpleTV.Http.Request(session, {method = 'post', url = decode64('aHR0cHM6Ly9hcGktd2ViLXpvcnBsYXkucGxhdGZvcm0yNC50di92Mi91c2Vycw'), body = body, headers = headers})
			if rc ~= 200 or not answer then return end
		
		local body1 = '{"login":"' .. login .. '","password":"' .. pass .. '","app_version":"v30"}'
		local rc, answer = m_simpleTV.Http.Request(session, {method = 'post', url = decode64('aHR0cHM6Ly9hcGktd2ViLXpvcnBsYXkucGxhdGZvcm0yNC50di92Mi9hdXRoL2xvZ2lu'), body = body1, headers = headers})
		...
		local rc, answer = m_simpleTV.Http.Request(session, {method = 'post', url = decode64('aHR0cHM6Ly9hcGktd2ViLXpvcnBsYXkucGxhdGZvcm0yNC50di92Mi91c2Vycy9zZWxmL2RldmljZXM/YWNjZXNzX3Rva2VuPQ') .. user_token, body = body2, headers = headers})
		...
		local rc, answer = m_simpleTV.Http.Request(session, {method = 'post', url = decode64('aHR0cHM6Ly9hcGktd2ViLXpvcnBsYXkucGxhdGZvcm0yNC50di92Mi9hdXRoL2RldmljZQ'), body = body3, headers = headers})
		local device_token = answer:match('access_token":"([^"]+)')
				if rc ~= 200 or not device_token then return end
		
		m_simpleTV.Config.SetValue('zor_token', device_token)
		
		return device_token
	end
	
	local access_token = m_simpleTV.Config.GetValue('zor_token')
	...
	local function GetStream(num, access_token)
		local url = decode64('aHR0cHM6Ly9hcGktd2ViLXpvcnBsYXkucGxhdGZvcm0yNC50di92Mi9jaGFubmVscy8') .. num .. '/stream?access_token='
		local rc, answer
	...
	local lastQuality = tonumber(m_simpleTV.Config.GetValue('zor_qlty') or 20000)
	local index = #t
	if #t > 1 then
		t[#t + 1] = {}
		t[#t].Id = 20000
		...
		if m_simpleTV.Control.MainMode == 0 then
			...
			t.ExtParams = {LuaOnOkFunName = 'zorSaveQuality'}
	...
	function zorSaveQuality(obj, id)
		m_simpleTV.Config.SetValue('zor_qlty', id)
	end
