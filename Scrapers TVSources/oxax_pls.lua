-- скрапер TVS для загрузки плейлиста "ОХ-АХ" http://oxax.tv (9/7/19)
...
-- ##
	local my_src_name = 'ОХ-АХ'
	module('oxax_pls', package.seeall)
	...
	function GetSettings()
		local scrap_settings = {name = my_src_name, sortname = '', scraper = '', m3u = 'out_' .. my_src_name .. '.m3u', logo = '..\\Channel\\logo\\Icons\\oxax.png', TypeSource = 1, TypeCoding = 1, DeleteM3U = 1, RefreshButton = 1, AutoBuild = 0, AutoBuildDay = {0, 0, 0, 0, 0, 0, 0}, LastStart = 0, TVS = {add = 1, FilterCH = 1, FilterGR = 1, GetGroup = 1, LogoTVG = 1}, STV = {add = 0, ExtFilter = 1, FilterCH = 1, FilterGR = 1, GetGroup = 1, HDGroup = 1, AutoSearch = 1, AutoNumber = 1, NumberM3U = 0, GetSettings = 0, NotDeleteCH = 0, TypeSkip = 1, TypeFind = 1, TypeMedia = 0}}
	...
	local function LoadFromSite()
		...
		local url = 'http://oxax.tv'
		local rc, answer = m_simpleTV.Http.Request(session, {url = url .. '/vz_spi.php'})
		m_simpleTV.Http.Close(session)
			if rc ~= 200 then return end
		local i, t = 1, {}
		local adr, title
			for w in answer:gmatch('<div class.-</div>') do
				adr = w:match('href="(.-)"')
				title = w:match('title="(.-)"')
