-- скрапер TVS для загрузки плейлиста "ZOR UZ" https://zorplay.uz (15/12/25)
-- Copyright © 2017-2025 Nexterr, NEKTO666 | https://github.com/Nexterr-origin/simpleTV-Scripts
-- ## необходим ##
-- видеоскрипт: zorplayuz.lua
-- ## Переименовать каналы ##
local filter = {
	--{'Setanta Sports Plus', 'Setanta Sports+'},
	}
	local my_src_name = 'ZOR UZ'
	module('zorplayuz_pls', package.seeall)
	...
	local function LoadFromSite()
		...
		local function GetChannels(access_token)
			local url = decode64('aHR0cHM6Ly9hcGktd2ViLXpvcnBsYXkucGxhdGZvcm0yNC50di92Mi9jaGFubmVscy9jaGFubmVsX2xpc3Q/YWNjZXNzX3Rva2VuPQ')
			local rc, answer
		...
		local t = {}
			for i = 1, #tab do
				if tab[i].is_free then
					...
					if not title:match('ZO\'R SPORT') then
						if slug and id and title then
							t[#t + 1] = {}
							t[#t].name = unescape3(title)
							t[#t].address = 'https://zorplay.uz/channels/' .. slug .. '/' .. id
