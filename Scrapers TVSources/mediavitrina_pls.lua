-- скрапер TVS для загрузки плейлиста "Витрина ТВ" https://vitrina.tv (6/2/26)
	...
	function LoadFromSite()
		...
		local t1 = {}
			for i = 1, #t do
				...
				if rc == 200 then
					local egress = answer:match('egress_version_id=([^&]%d*)')
					for title, adr in answer:gmatch('{%s*"title":%s*"([^"]+)"%s*,%s*"streams_api_url":%s*"([^"%?]+)') do
						...
						t1[#t1].name = title
						t1[#t1].address = adr:gsub('\\/', '/') .. '?egress=' .. egress
						t1[#t1].logo = t[i].logo
						plusCh = true
					end
				end
