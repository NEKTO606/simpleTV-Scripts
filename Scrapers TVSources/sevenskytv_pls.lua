-- скрапер TVS для загрузки плейлиста "SevenSky TV" https://tv.seven-sky.net (24/2/26)
-- Copyright © 2017-2026 Nexterr, NEKTO666 | https://github.com/Nexterr-origin/simpleTV-Scripts
-- ## необходим ##
-- видеоскрипт: sevenskytv.lua
-- ## Переименовать каналы ##
local filter = {
		--{'РБК ТВ HD', 'РБК HD'},
	}
	local host = 'https://7sky.server-api.lfstrm.tv/'
	local my_src_name = 'SevenSky TV'
	module('sevenskytv_pls', package.seeall)
	...
	function GetSettings()
	 return {name = my_src_name, sortname = '', scraper = '', m3u = 'out_' .. my_src_name .. '.m3u', logo = '..\\Channel\\logo\\Icons\\sevenskytv.png', TypeSource = 1, TypeCoding = 1, DeleteM3U = 1, RefreshButton = 1, show_progress = 0, AutoBuild = 0, AutoBuildDay = {0, 0, 0, 0, 0, 0, 0}, LastStart = 0, TVS = {add = 1, FilterCH = 1, FilterGR = 1, GetGroup = 1, LogoTVG = 0}, STV = {add = 1, ExtFilter = 1, FilterCH = 1, FilterGR = 1, GetGroup = 1, HDGroup = 1, AutoSearch = 1, AutoNumber = 1, NumberM3U = 0, GetSettings = 1, NotDeleteCH = 0, TypeSkip = 1, TypeFind = 1, TypeMedia = 0, RemoveDupCH = 1}}
	end
	function GetVersion()
	 return 2, 'UTF-8'
	end
	...
	local function GetToken()
		local saveToken = m_simpleTV.Config.GetValue('svnsktv_token')
		local tok
		if saveToken and CheckToken(saveToken) == 200 then
			...
			local rc, answer = m_simpleTV.Http.Request(session, {url = decode64('aHR0cDovL285Njg4OW5vLmJlZ2V0LnRlY2gvc3Zuc2t0di5waHA'), headers = headers})
			if rc ~= 200 then return end
				if answer then
					answer = decode64(answer)
					if CheckToken(answer) == 200 then
						tok = answer
						m_simpleTV.Config.SetValue('svnsktv_token', tok)
					else
	...
	local tvtoken = GetTvAssetToken()
		if not tvtoken then return end
	
	local function GetJson(url)
		local rc, answer = m_simpleTV.Http.Request(session, {url = host .. url .. '?tv-asset-token=' .. tvtoken})
			if rc ~= 200 then return end
		answer = answer:gsub('\\', '\\\\')
		answer = answer:gsub('\\"', '\\\\"')
		answer = answer:gsub('\\/', '/')
			...
			for i = 1, #channels do
				for _, v in pairs(m) do
					if channels[i].id == v.media then
						...
						if id and title and rc == 200 then
							t[#t + 1] = {}
							t[#t].name = unescape3(title)
							t[#t].address = 'https://tv.seven-sky.net/' .. id
